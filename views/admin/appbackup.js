...
app.get('/adminproducts/:id', function(req,res) {
  ...
  // });
  client.query('SELECT products.brand_id AS brand_id, products.category_id AS category_id, products.price AS price, products.specification AS specification, brands.brand_name AS brand_id, categories.category_name AS category_name FROM products INNER JOIN brands ON products.brand_id=brands.id INNER JOIN categories ON products.category_id=categories.id WHERE products.id='+req.params.id+';')
  .then((result)=>{
